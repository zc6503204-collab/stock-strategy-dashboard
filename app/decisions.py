...
def check(signal,q,validation,account,cfg,t,active_versions=None):
    ...
    base={'signal_id':signal.id,'symbol':signal.symbol,'strategy':signal.strategy,'version':signal.version,
          ...
          'target':signal.trigger+2*(signal.trigger-signal.stop),'risk_group':signal.risk_group,
...
def holding_plan(p,q,t,validation):
    ...
    try:
        sessions=session_count(entryday,d,m)
        due=sessions>=3 and (sessions>3 or t>=close_time(d,m)-timedelta(minutes=10))
    ...
    elif due:action='到期退出';reason='已到第三交易日退出准备时间';event='time_exit'
    elif fresh and price>=p['target'] and not p.get('partial'):
        action='减仓止盈';reason='达到两倍初始价格风险的止盈目标';event='take_profit'
