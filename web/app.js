...
function strategyName(value){return ({breakout:'开盘区间突破',pullback:'首次回调再启动',trend_pullback:'趋势回踩再启动',volatility_breakout:'波动收缩突破',trend_rsi_pullback:'趋势RSI回踩',vcp_swing:'VCP波动收缩突破',orb20_us:'美股20分钟开盘突破'}[value]||value||'等待盘中形态')}
...
function renderToday(){
  ...
  renderAutoResearch(w);
  const s=d.session_summary||{},statusText=d.status==='BUY'?'当前可买':d.status==='MANAGE'?'先处理持仓':s.state==='running'?'自动扫描中':'本轮无买点';
  const scanProgress=currentMarket==='CN'?{done:w.research?.checked||0,total:w.research?.supported||0}:s.scan_progress;
  const progress=s.refreshing&&scanProgress?.total?` · 日线复核 ${scanProgress.done}/${scanProgress.total}`:'';
...
function renderHoldings(){
  ...
  else $('#holdings-content').innerHTML=`<table class="holdings-table"><thead><tr><th>股票 / 策略</th><th>周期</th><th>现价</th><th>买入价</th><th>剩余数量</th><th>保护价</th><th>第一止盈</th><th>当前动作</th><th>行情时间</th></tr></thead><tbody>${rows.map(row=>`<tr><td><b>${esc(row.name||row.symbol)}</b><small>${esc(row.symbol)} · ${esc(strategyName(row.strategy))}</small></td><td>${esc(horizonName(row.horizon))}</td><td>${money(row.price)}</td><td>${money(row.entry)}</td><td>${num(row.remaining,0)}股</td><td>${money(row.stop)}</td><td>${money(row.target)}</td><td><span class="action-chip ${row.event?'urgent':''}">${esc(row.action)}</span><small>${esc(row.reason)}</small></td><td>${esc(time(row.quote_time,w.meta.timezone))}</td></tr>`).join('')}</tbody></table>`;
}
...
function bindEvents(){
  ...
  $('#settings-form').addEventListener('submit',async event=>{event.preventDefault();await action('/api/settings',{account_mode:!!state.settings.account_mode,monitor_limit:Number($('#monitor-limit').value),simulation_enabled:state.settings.simulation_enabled,ib_port:Number($('#ib-port').value)},'运行设置已保存')});
...
function renderAutoResearch(w){
  const panel=$('#auto-research-panel');if(!panel)return;panel.classList.toggle('hidden',w.meta.market!=='CN');if(!w.research)return;
  const r=w.research,labels={running:'逐股复核中',enumerating:'更新证券库',complete:'本轮覆盖完成',partial:'部分数据待补齐',error:'数据请求失败',interrupted:'扫描中断待恢复'};
  const supported=r.supported||0,checked=r.checked||0;
  panel.innerHTML=`<div class="panel-head"><div><span class="kicker">每日自动研究</span><h2>${esc(labels[r.state]||'等待首次扫描')}</h2><p>${esc(r.reason||'9:10准备，盘中持续筛选，15:10形成日报')}</p></div><span class="action-chip">${state.monitor.running?'自动运行':'已暂停'}</span></div>
    <div class="research-counts"><div>证券库<b>${num(r.enumerated??r.universe_total??0,0)} / ${num(r.universe_total??0,0)}</b></div><div>支持范围<b>${num(supported,0)}</b></div><div>日线已复核<b>${num(checked,0)}</b></div><div>缺日线<b>${num(r.missing||0,0)}</b></div><div>有效研究候选<b>${num(Math.max(0,(r.research_pool||0)-(r.pool_data_missing||0)),0)} / 80</b></div><div>实时监测<b>${num(w.health.tracked,0)} / ${num(r.monitor_limit,0)}</b></div></div>
    <progress max="${Math.max(1,supported)}" value="${checked}"></progress><p class="form-hint">${esc(r.scope)} · 数据截至 ${esc(r.cutoff||'待核验')} · 保护名额 ${num(r.protected_slots||0,0)} · 研究池缺实时核验 ${num(r.pool_data_missing||0,0)} · ${r.schedule_delayed?'上一轮未完成，新任务已合并等待':'按交易时段自动调度'}。页面分数为准备度，不代表胜率。</p>
    <p class="form-hint">${esc(r.runtime_validation?.message||'正在积累实机运行记录')} · 已验证 ${r.runtime_validation?.verified_days?.length||0} / 2 个完整交易日。</p>
    <details><summary>最新候选变化与仍缺的研究条件</summary>${(r.changes||[]).slice(0,8).map(c=>`<div class="event-row"><b>${esc(c.name||c.symbol)}</b><span>${esc(c.state)} · ${esc(c.reason)}</span><button class="text-button" data-history="${esc(c.symbol)}">历史</button></div>`).join('')||'<p>本轮尚未产生候选变化。</p>'}<p>待补充：${esc((r.missing_document_checks||[]).join('、'))}。现有信号仍是实验规则结果。</p></details>
    ${(r.reports||[]).slice(0,2).map(report=>researchReport(report)).join('')}`;
}
