...
灵犀负责按策略查询 A 股市场，长桥补充行业和全市场活跃候选，本地程序再用同源日线重算趋势、流动性、相对强弱和风险。A股、美股核心池每个交易日重建；榜单异动只参与当天排序。同行业第三只起每只软扣4分，但不设置行业硬上限。上图为示例界面，市场数据仅用于展示功能。
...
A股、美股盘前分别重建最多80只的研究池；交易时段每5分钟只更新异动补充并重新排序，次日不会沿用。刷新失败时保留最近有效快照并标记过期。最多12只重点股票使用行情推送，并以15秒查询补充。买点只在完整5分钟K线结束后确认，止损和止盈可按有效实时行情触价提醒。
