...
| [LEAN](https://github.com/QuantConnect/Lean) | 策略信号、组合构建、风险和执行模块可替换 | 七种策略统一注册、统一输出信号，再由同一资金与风控层决定是否可买 |
...
| [trading-orb](https://github.com/sam-bateman/trading-orb) | 美股开盘区间形成、限定入场时段和日内退出的研究框架 | 独立实现美股20分钟开盘区间：10:00—11:30确认，固定区间止损止盈，15:50退出 |
| [Qullamaggie Breakout Scanner](https://github.com/VladPetrariu/Qullamaggie-breakout-scanner) | 强势股、均线排列、接近高点和波动收缩的候选过滤思路 | 独立实现VCP波段策略，并加入260根预热、同市场RS排名、ATR风险上限和10日退出 |
| [tick-stock-panel](https://github.com/shy3130/tick-stock-panel) | RSI中轴、均线趋势和回踩观察的公开思路 | 独立实现趋势RSI回踩，并要求先出现强势、再缩量回撤、最后由盘中VWAP/EMA和量能确认 |
...
- 不把仓库README、单次回测或高胜率标题当作有效性证据。每种策略分别保留影子前向、受资金约束的组合模拟和历史探索三套结果，并用样本量与95%区间展示不确定性。
